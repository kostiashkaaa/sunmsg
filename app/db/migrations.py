...
def run_migrations() -> None:
    ...
    try:
        ...
        add_column_if_missing(conn, cursor, 'users', 'group_invite_privacy', "group_invite_privacy TEXT NOT NULL DEFAULT 'all'")
        ...
        if not migration_applied(cursor, USER_PROFILE_NORMALIZATION_MIGRATION[0]):
            ...
            conn.commit()
        if not migration_applied(cursor, GROUP_INVITE_REQUESTS_MIGRATION[0]):
            cursor.execute(
                '''
                UPDATE users
            ...
            cursor.execute(
                ...
                ON CONFLICT(version) DO NOTHING
                ''',
                GROUP_INVITE_REQUESTS_MIGRATION,
            )
            conn.commit()
