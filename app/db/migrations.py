...
APNS_DEVICE_TOKENS_MIGRATION = (38, 'create_apns_device_tokens_table')
...
def _run_apns_device_tokens_migration(conn, cursor) -> None:
    if migration_applied(cursor, APNS_DEVICE_TOKENS_MIGRATION[0]):
        return
    ...
    cursor.execute(
        ...
        CREATE TABLE IF NOT EXISTS apns_device_tokens (
            ...
            user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
            token TEXT NOT NULL,
            push_type TEXT NOT NULL DEFAULT 'voip',
            ...
            is_active INTEGER NOT NULL DEFAULT 1,
            UNIQUE(token, push_type)
        )
        '''
    )
    cursor.execute(
        '''
        CREATE INDEX IF NOT EXISTS idx_apns_device_tokens_user_type_active
        ON apns_device_tokens(user_id, push_type, is_active)
        '''
    )
    cursor.execute(
        '''
        CREATE INDEX IF NOT EXISTS idx_apns_device_tokens_updated_at
        ON apns_device_tokens(updated_at DESC)
        '''
    )
    _record_migration(cursor, APNS_DEVICE_TOKENS_MIGRATION)
    conn.commit()
    logger.info('Migration: created apns_device_tokens table')
...
def run_migrations() -> None:
    ...
    try:
        ...
        _run_apns_device_tokens_migration(conn, cursor)
        conn.commit()
