...
def test_stage_private_key_for_redirect_uses_session_bound_persistence_from_bootstrap():
    script_path = Path(__file__).resolve().parents[1] / 'static' / 'modules' / 'private-key-session-bridge.js'
...
let capturedOptions = null;
const windowTarget = new FakeEventTarget();
windowTarget.SUN_BOOTSTRAP = {{
  session: {{
    autoLogoutSeconds: 604800,
...
if (
  ...
  throw new Error(`Expected session-bound persistent metadata, got ${{JSON.stringify(capturedOptions)}}`);
}}
"""
